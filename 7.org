...
*** 7.15. Legacy Record Types
 ...
 标记 (~#n=~) 和引用 (~#n#~) 句法可在 /record/ 句法内使用，用于根据意图创建共享的或环状的结构。不过，所有环必须是可解析的，不能改变一个不可变 /record/ 字段。TODO
 ...
 These identifiers are auxiliary keywords for define-record. It is a syntax violation to reference these identifiers except in contexts where they are recognized as auxiliary keywords. mutable and immutable are also auxiliary keywords for define-record, shared with the R^{6}RS 报告 define-record-type.
 ...
 This form is equivalent to the R^{6}RS 报告 record-type-descriptor form.
