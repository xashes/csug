...
*** 7.15. 传统的 record 类型 (Legacy Record Types)
 ...
 /Record/ 可以通过 ~define-record~ 语法形式或通过 ~make-record-type~ 过程定义。在 R^{6}RS 报告中的机制和替代机制中， /record/ 的底层表示和类型描述符是一样的。通过过程机制，一种机制创建的 /record/ 类型可以作为另一种机制创建的 /record/ 类型的父类型，不过通过语法机制则无法做到。
 ...
 这些标识符是 ~define-record~ 的辅助关键字。除了在被识别为辅助关键字的上下文中，在其它任何地方引用这些标识符都是违反语法的。 ~mutable~ 和 ~immutable~ 也是 ~define-record~ 的辅助关键字，和 R^{6}RS 报告中的 ~define-record-type~ 一样。
 ...
 这个语法形式等价于 R^{6}RS 报告中  ~record-type-descriptor~  的语法形式。
