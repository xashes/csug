...
*** 7.9. 符号 (Symbols)
 ...
 当环境参数被设为 ~#f~, 则 /printer/ 只输出美观名字，所以
 ...
 当生成符号被频繁创建，但很少被输出或存储在一个对象文件中时，为了减小构造和（线程）同步消耗，美观和唯一名字的生成是惰性的，即，直到第一次被 /printer/, /fasl writer/, 或显式地被过程 ~symbol->string~ 或 ~gensym->unique-string~ 请求时才创建。此外，在唯一名字被请求之前，生成符号不会被放入系统的驻留符号表（oblist; 参见第 156 页）。这使得一个生成符号可以在下列情况下被存储管理器回收：如果不存在对这个生成符号的引用，且不存在可访问的唯一名字，即使它有一个顶层绑定，或非空的属性列表。
 ...
 如上所述， /Chez Scheme/ 把美观名字的创建延迟到其第一次被请求时──被 /printer/ 或通过显式调用 ~symbol->string~ 。在那之前，这些环境参数并不会起作用；因此，在调用 ~gensym~ 时设置它们，对生成的名字并没有影响。
...
*** 7.13. Record Types
 ...
 /Chez Scheme/ 以一种方式扩展了 R^{6}RS 中的 ~define-record-type~ 句法，其通过包含一个以 ~#f~ 作为 /uid/ 值的 ~nongenerative~ 子句，支持显式声明一个生成的 /record/ 类型（以一种类似双重否定的方式），即，
 ...
 这种方式可以和环境参数 ~require-nongenerative-clause~ 结合使用，
...
*** 7.14. Record Equality and Hashing
 ...
 默认情况下， ~equal?~ 使用 ~eq?~ 比较 /record/ 实例，即，它区分不是同一个对象 (non-eq?) 的实例，即使它们是同一个类型且有相同的内容。对一个 /record/ 类型（及它没有实现自己专有的相等性比较过程的子类型）的实例，程序可以通过使用 ~record-type-equal-procedure~ 来把相等性比较过程和描述这种 /record/ 类型的 /record/ 类型描述符(rtd)关联起来，以重载这种行为。
 ...
 通过环境参数 ~default-record-equal-procedure~ 可以指定一个用于所有 /record/ 类型（包括不透明类型）的默认相等性过程。只有在实例所属类型没有实现或继承一个类型特定的相等性过程时，这个默认过程才会被使用。
 ...
 类似的，当 ~equal-hash~ 简单地散列化一个 /record/ 实例，它默认为一个独立于实例所属类型和内容的值。对于一个 /record/ 类型的实例，通过使用 ~record-type-hash-procedure~ 把散列过程和描述这个 /record/ 类型的 /record/ 类型描述符（rtd）关联起来，程序可以重载这种行为。 ~record-hash-procedure~ 过程可以沿着继承链，找到一个特定 /record/ 实例的散列过程。 /equal-hash/ 传递给散列过程 2 个参数：实例，和一个可用于递归地散列化实例中的值的散列过程。递归地进行散列化是必要的，这样才能散列化潜在的循环结构，并使得对共享结构的散列化更为高效。
 ...
 通过环境参数 ~default-record-hash-procedure~ 可以指定一个用于所有 /record/ 类型（包括不透明类型）的默认散列过程。只有当实例所属类型没有实现或继承一个类型特定的散列过程时，这个默认的散列过程才会被使用。

 以下的例子说明了相等性过程和散列过程的设置。
 ...
 下面的例子说明了相等性过程和散列过程在循环 /record/ 结构上的应用。
 ...
 在第一种形式中， ~equal-proc~ 必须是一个过程或 ~#f~. 如果 ~equal-proc~ 是一个过程，则 ~rtd~ 和 ~equal-proc~ 之间会建立一个新的关联，替代现有的任何此种关联。如果 ~equal-proc~ 是 ~#f~, 任何现存的 ~rtd~ 和相等性过程之间的关联都会被解除。

 第二种形式中， ~record-type-equal-procedure~ 返回 ~rtd~ 关联的相等性过程，如果不存在，则返回 ~#f~.

 当改变一个 /record/ 类型的相等性过程时，如果这个 /record/ 类型有散列过程，则需要的话也应该更新，以确保为当前相等性过程认为相等的任意两个实例生成相同的散列值。
