...
*** 7.9. 符号 (Symbols)
 ...
 唯一名字使得 /gensym/ 的字面形式可以被读回及在输入端可靠地一般化。 /gensym/ 的句法包括美观名字和唯一名字，如下面的例子所示：
 ...
 当读取器遇到 ~#:~ 句法，便以其提供的美观名字产生一个 /gensym/ ，但是最初的唯一名字就遗失了。
...
*** 7.15. Legacy Record Types
 ...
 除了 R^{6}RS 报告中的 record 类型的创建和定义机制——参见“The Scheme Programming Language, 第 4 版”，第 9 章中的讲解， /Chez Scheme/ 仍然支持之前版本中创建新数据类型，或称 /record/ 类型（带有固定数量的命名字段）的机制。本节中介绍的很多过程，只在从 ~(chezscheme csv7)~ 库中导入 (import) 之后才可用。

 Code intended to be portable should use the R6RS mechanism instead.
 若代码要考虑移植性的话，应该使用 R6RS 机制。

 Records may be defined via the define-record syntactic form or via the make-record-type 过程. The underlying representation of records and record-type descriptors is the same for the R^{6}RS 报告 mechanism and the alternative mechanism. Record types created by one can be used as parent record types for the other via the procedural mechanisms, though not via the syntactic mechanisms.make
 /Record/ 可以通过 ~define-record~ 句法形式或通过 ~make-record-type~ 过程定义。在 R^{6}RS 报告中的机制和替代机制中， /record/ 的内在表示和类型描述符是一样的。通过过程机制，一种机制创建的 /record/ 类型可以作为另一种机制创建的 /record/ 类型的父类型，不过通过句法机制则无法做到。

 句法接口 (~define-record~) 是最常用的接口。每个 ~define-record~ 句法形式定义一个新 /record/ 类型的构造过程，一个只在对象为此类型时返回 ~#t~ 的类型谓词，针对各个字段的访问过程，针对各可变字段的赋值过程。例如，

 #+BEGIN_SRC scheme
   (define-record point (x y))
 #+END_SRC

 创建一个有两个字段，x 和 y 的 ~point~ /record/ 类型，同时定义了下列过程：
 ...
 过程接口 (~make-record-type~) 可以用于实现必须处理 ~define-record~ 句法形式的解释器。对 ~make-record-type~ 的每次调用，返回一个表示此 /record/ 类型的 /record/ 类型描述符。 使用此 /record/ 类型描述符，程序可以动态生成构造过程，类型谓词，字段访问过程，及字段修改过程。以下的代码说明了如何使用过程接口创建一个类似的 ~point~ /record/ 类型，及相关的定义。
 ...
 过程接口比句法接口更灵活，但这种灵活性会导致程序的可读性较差，也会降低编译器生成高效代码的能力。只要句法接口能完成任务，程序员应该尽可能使用它。
 ...
 /record/ 类型描述符也可以从 /record/ 类型的实例中提取，无论这个类型是通过 ~define-record~ 还是通过 ~make-record-type~ 生成的，而且提取出的描述符也可用于生成构造过程，谓词，访问器，和修改器，不过有如下关于 ~record-type-descriptor~ 的描述中所注明的一些局限。这是个强大的特性，使得编写可移植的打印器和对象检查器成为可能。例如，在默认 /record/ 打印器中使用此特性的打印器，以及通过它在 /debug/ 期间支持检查和修改系统或用户定义的 /record/ 的检查器。
 ...
 一个父 /record/ 类型可以在 ~define-record~ 句法中指明，或作为 ~make-record-type~ 的一个可选参数。一个新的 /record/ 继承父 /record/ 的字段，而且每个新 /record/ 类型的实例也被认为是父类型的实例，所以父类型的访问器和修改器也可用于新类型的字段。
 ...
 A new type results for each generative record definition, while only one type results for all occurrences of a given nongenerative record definition. This distinction is important semantically since record accessors and setters are applicable only to objects with the same type.
 ...
 句法接口 (~define-record~) 的 /record/ 定义，默认是在展开期生成的，即，一个新的 /record/ 类型是在代码展开期创建的。当交互输入，从源码载入，或用编译文件编译时，对每个句法形式，展开在编译或解释之前进行一次。因此，对单个 ~define-record~ 句法形式的多次求值，例如，在一个被调用多次的过程体中，也总是生成相同的 /record/ 类型。

 不在同一处的 ~define-record~ 句法形式通常生成不同的类型，即使其形式在文本上是一样的。唯一的例外是当 /record/ 的名字被指定为一个生成的符号时，即 /gensym/ (参见 152 页)。一个名字由 /gensym/ 指定的 /record/ 定义，其多份拷贝也总是生成同一个 /record/ 类型，即，这种定义是 /nongenerative/ 的。 /record/ 定义的每份拷贝必须包含相同顺序的同样字段和字段修饰符；当拥有同样的生成名字的两个不同的 /record/ 类型被加载进同一个 Scheme 进程时，一个条件类型的 ~&assertion~ 异常会被抛出。

 过程接口 (~make-record-type~) 的 /record/ 定义默认是运行时生成的。即，对 ~make-record-type~ 的每个调用通常都会生成一个新的 /record/ 类型。和句法接口一样，唯一的例外是当类型名是以 /gensym/ 方式指定时，这种情况下， /record/ 类型是完全 /nongenerative/ 的。
 ...
 一个给定类型的 /record/ 的置信打印形式，可以通过 ~record-writer~ 重载。
 ...
 默认的 /record/ 句法也能用作读取器的输入，只要在读取器所在的 Scheme 会话中，对应的 /record/ 类型已经被定义。环境参数 ~record-reader~ 可以被用于指定一个替代生成的名字的，供读取器辨识的不同名字。以这种方式指定一个不同的名字，同时改变了 /record/ 打印出的名字。读取器若遇到 ~#!r6rs~ ，则会在其后的输入流中禁用这些扩展，除非在更近的位置遇到 ~#!chezscheme~.

 标记 (~#n=~) 和引用 (~#n#~) 句法可在 /record/ 句法内使用，用于根据意图创建共享的或环状的结构。不过，所有环必须是可解析的，不能改变一个不可变 /record/ 字段。即，任何环必须包含至少一个指向一个可变字段的指针，不论它是个可变的 /record/ 字段，还是一个内置对象类型（如点对或向量）的可变字段。
 with the result of creating shared or cyclic structure as desired. All cycles must be resolvable, however, without mutation of an immutable record field. That is, any cycle must contain at least one pointer through a mutable field, whether it is a mutable record field or a mutable field of a built-in object type such as a pair or vector.

 When the parameter print-record is set to #f, records are printed using the simpler syntax
 当环境参数 ~print-record~ 被设置为 ~#f~ 时， /record/ 使用更简单的句法打印
 ...
 句法: (define-record name (fld1 ...) ((fld2 init) ...) (opt ...))

 句法: (define-record name parent (fld1 ...) ((fld2 init) ...) (opt ...))

 返回: 未定义

 库: (chezscheme)

 ~define-record~ 句法形式是一种定义，可以出现在，也只能出现在一切其它定义可以出现的地方。
 ...
 每个 ~fld~ 必须是一个字段名标识符，或必须采用以下句法形式

 ~(class type field-name)~

 ~class~ 和 ~type~ 是可选的，而 ~field-name~ 是一个标识符。如果提供 ~class~, 则必须指定不可变关键字或可变关键字。如果指定不可变 class 批示符，则字段是不可变的；否则，字段是可变的。 ~type~, 如果存在的话，则指定字段如何表示，如下所示。
 ...
 如果指定一个类型，则字段只能包含指定类型的对象。如果没有指定类型，则字段为 ~ptr~ 类型，意思是它可以包含任意 Scheme 对象。
 ...
 如果指定了 ~parent~ ，则 ~parent~ 命名的类型为 /record/ 的父类型。新的类型继承父类型的所有字段，并且新类型的实例同时也被判定为父类型。如果未指定 ~parent~ ，则父类型为一个没有字段的基础 /record/ 类型。
 ...
 如果指定的父类型，则父类型的参数排在前面，同时，在 /record/ 中，父类型的字段也插入到子类型的字段前面。
 ...
 这个例子说明了指定读取器命名，不可变字段，以及图形标记和引用句法的用法。
 ...
 即使程序的不同组件是从不同的源文件加载的，或从不同目标文件加载且是分别编译的，这也可以有效运行。
 ...
 这些标识符是 ~define-record~ 的辅助关键字。除了在被识别为辅助关键字的上下文中，在其它任何地主引用这些标识符都是违反句法的。 ~mutable~ 和 ~immutable~ 也是 ~define-record~ 的辅助关键字，和 R^{6}RS 报告中的 ~define-record-type~ 一样。


 句法: (type-descriptor name)
 ...
 这个句法形式等价于 R^{6}RS 报告中  ~record-type-descriptor~  的句法形式。

 /record/ 类型描述符在使用 ~record-reader~ 和 ~record-writer~ 重载默认读写句法时很有用，也可和本节后面将要讲述的过程接口 routines 配合使用。
 ...
 传入参数 ~name~ 和 ~rtd~ 时， ~record-reader~ 把 ~rtd~ 注册为 record 类型描述符，用于在读取过程遇到一个用 ~name~ 命名的 record，并用默认 record 句法打印时。
 ...
 当只传入一个参数时，record-writer 返回 rtd 关联的 /record writer/, 对所有 record 来说，最初都是默认 /record writer/. 默认打印方法以统一的句法打印所有的 record，包括生成的 record 名字，各字段的值，一如在本节的介绍部分描述的。

 当传入两个参数时， ~record-writer~ 在 ~rtd~ 和 ~procedure~ 之间建立一个新的关联，从而使过程被用于打印器，以代替给定类型的 record 的默认打印器。打印器传给 ~procedure~ 三个参数：record /r/, 端口 /p/, 和一个应该被用于输出任意 Scheme 对象的打印方法选择包含在 record 打印形式中的值的过程 /wr/, 即，record 字段的值。
 ...
 只有当 ~print-record~ 为 ~#t~ 时（默认为真）， /record writer/ 才被使用。当参数 ~print-record~ 被设为 ~#f~ 时，record 使用一种只指明 record 类型的精简句法打印。
 ...
 这个参数控制 record 的打印形式。如果设为真（为默认值），则使用与 record 类型相关联的 /record writer/ 打印此类型 record。如果设为假，所有 record 都以句法 ~#<record of type name>~ 打印，其中 ~name~ 是 record 类型的名称，即 ~record-type-name~ 的返回值。
 ...
 其中 ~class~ 和 ~type~ 是可选的。 ~field-name~ 必须是一个符号。如果指定 ~class~ ，则必须是符号 ~immutable~ 或符号 ~mutable~ 。如果提供 ~immutable~ 类别指示符，则字段是不可变的；否则，字段是可变的。 ~type~ ，如果提供的话，指定了字段如何表示。类型种类和 174 页上对 ~define-record~ 的描述中给出的一样。
 if present, must be the symbol immutable or the symbol mutable. If the immutable class-specifier is present, the field is immutable; otherwise, the field is mutable. type, if present, specifies how the field is represented. The types are the same as those given in the description of define-record on page 174.
 ...
 The record-type descriptor may be passed as an argument to any of the R^{6}RS 报告 procedures
 record 类型描述符可以作为参数传递给 R^{6}RS 报告中的下列任何过程
 ...
 This 过程 is equivalent to the R^{6}RS 报告 record-rtd 过程.
